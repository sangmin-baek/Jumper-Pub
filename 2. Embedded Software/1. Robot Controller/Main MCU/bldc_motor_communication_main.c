...
_BLDC_DATA_uSD _bldc_data_uSD;

float uSD_data_threshold[17] = {20, 20,			// current d
								20, 20,			// current q
								1e10, 1e10,		// angle
								5e5, 5e5,		// velocity
								1e10,			// elec angle
								10, 10,			// target currents
								1e10, 5e5,		// target angle & velocity
								20, 20, 		// pid currents
								1e10, 5e5};		// pid angle & velocity



void Init_BLDC_Data_uSD()
{
	for(uint8_t i = 0; i < BLDC_SPI_DATA_NUM; i++)
	{
		_bldc_data_uSD.uSD_data[i] = 0;
	}
}
...
void Process_BLDC_Data_RX(){
	...
	for(uint8_t i = 0; i < BLDC_SPI_DATA_NUM; i++)
	{
		...
		if((_bldc_data.bldc_data_float[i] < uSD_data_threshold[i]) && (_bldc_data.bldc_data_float[i] > -uSD_data_threshold[i]))
		{
			_bldc_data_uSD.uSD_data[i] = _bldc_data.bldc_data_float[i];
		}
	}
